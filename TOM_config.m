...
function [r] = TOM_config(DCM,options,subject,laplace)
...
if laplace == true
    ...
    r.objfunhan = @FBT_llfun_fminunc;
    r.objfun = 'FBT_llfun_fminunc';
end
...
init_mu = invsigmtr(DCM.priors,r.LB,r.UB,50); %Prior mean %invsigmtr(r.MB,r.LB,r.UB,50) returns 0
r.pfix = init_mu;

%prior std found in estimates from previous paper
% r.init_sig = invsigmtr((DCM.prior_std),r.LB,r.UB,50);
r.init_sig = 0.25*abs(invsigmtr(0.99*(r.UB-r.LB),r.LB,r.UB,50)); %Prior std
...
for sji=1:r.nsjs    
    ...
    % Find how many timeouts were there

    all_timeouts = [X.timeout]; % Concatenate 'timeout' values
    %timeout_percent = nansum(all_timeouts, 'all')/length(all_timeouts);
    timeout_percent = sum(all_timeouts(~isnan(all_timeouts))) / numel(all_timeouts);
    ...
    if sjind  %If fitting this subject's data
        ...
        r.subjects(sj).model_n = DCM.model_n;
        r.subjects(sj).timeout_percent = timeout_percent;
