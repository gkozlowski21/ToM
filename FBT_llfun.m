...
function [l,model_acc,avg_action_prob]=FBT_llfun(p,r,sj)
...
r.pfix(r.opt_idx) = p';
p_tr =  sigmtr(r.pfix,r.LB,r.UB,1);
...
%             RP_hat(n) = betarnd(real(betaA), real(betaB));
    ...
    if RP(n) ~= eps
        for z = 1:10        % Calculate the area under the curve for every individual bin(10 total)
            lower_bound = (z-1)/10;
            upper_bound = z/10;
            
            P_lower = betainc(lower_bound, real(betaA),real(betaB));  % CDF at lower bound
            P_upper = betainc(upper_bound, real(betaA),real(betaB)); % CDF at upper bound % Find the probability between the bounds
        
            P_interval(z) = P_upper - P_lower;
        end

        u = u+1;
%         lower_bound = floor(RP(n)*10)/10;
        upper_bound = ceil(RP(n)*10)/10;
        lower_bound = upper_bound - .1;
        P_lower = betainc(lower_bound, real(betaA),real(betaB));  % CDF at lower bound  
        P_upper = betainc(upper_bound,real(betaA),real(betaB));  % CDF at upper bound % Find the probability between the bounds 
        avg_action_prob(u) = P_upper-P_lower;
        max_index = find(P_interval ==(max(P_interval)));
        if any(max_index <= 5) && upper_bound <= .5
            model_acc(u) = 1;
        elseif any(max_index > 5) && lower_bound > .5
            model_acc(u) = 1;
        else
            model_acc(u) = 0;
        end
...
avg_action_prob = mean(avg_action_prob)
